...
class FirebaseAPI {
  ...
  // Retorna todos os usuários do sistema (para painel admin)
  async getAllUsers() {
    try {
      const usersData = await this.getData('usuarios');
      const usersArray = this.objectToArray(usersData);
      // Não retorna a senha!
      return usersArray.map(({ senha, ...user }) => user);
    } catch (error) {
